...
//        return $country;
    ...
    {
        return CarType::where('active',1)->get();
    }

    public function nationalTypes()
    {
        return NationalType::where('active',1)->get();
    }
    ...
    public function cities($input)
    {
        ...
        return City::where('active',1)
            ->with(['regions' => function($q)use($lang){
                $q->where('active',1)
                    ->select('id','name_'.$lang.' as name','city_id');
            }])
    ...
    public function setting($input)
    {
        ...
        $paymentWays = PaymentWay::where('active',1)
            ->where('accept',1)
            ->select('id','name_'.$lang.' as name')
            ->get();
        return [
