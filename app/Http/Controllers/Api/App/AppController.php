...
class AppController extends Controller
{
    ...
    {
        $data = $this->appRepository->setting($request);
        return response()->json(msgdata($request, success(), 'success', $data));
    }
    ...
    public function aboutUs(Request $request)
    {
        if ($request->header('lang') == 'ar')
            $about = $this->appRepository->aboutUs()
                ->select('image','body_ar as body')->first();
        else
            $about = $this->appRepository->aboutUs()
                ->select('image','body_en as body')->first();
    ...
    public function appExplanation(Request $request, $privacy = 0)
    {
        if ($request->header('lang') == 'ar'){
            if($privacy == 0){
