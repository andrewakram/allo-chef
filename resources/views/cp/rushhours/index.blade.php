                ...
                <div class="row">
                    <div class="col">
                        ...
                        <div style="float: left">
                            @if(auth()->user()->hasPermissionTo('اضافة توقيت'))
                        ...
                        <table class="table" id="myTable">
                            ...
                            @foreach($results as $c)
                                <tr id="main_cat_{{$c->id}}" onclick="myFunction({{$c->id}})">
                                    <td>{{$c->id}}</td>
                                    <td>{{$c->country_name_en}} / {{$c->country_name}}</td>

                                    <td>{{$c->from}}</td>
                                    <td>{{$c->to}}</td>

                                    <td>
                                        @if(auth()->user()->hasPermissionTo('تعديل توقيت'))
                                        <button title="تعديل" type="button" class="btn btn-warning" data-toggle="modal" data-target="#edit_{{$c->id}}">
                                            <i class="fa fa-edit"></i>
                                        </button>
                                        @endif

                                        @if(auth()->user()->hasPermissionTo('حذف توقيت'))
                                                ...
                                                <form class="form-horizontal" method="post" action="{{route('editRushhour')}}" enctype="multipart/form-data">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="form-group row {{ $errors->has('ar_name') ? ' has-error' : '' }}">
                                                            ...
                                                            <div class="col-lg-12">
                                                                <input  type="text" placeholder="الدولة" class="form-control btn-square" value="{{$c->country_name_en}} / {{$c->country_name}}" readonly>
                                                            </div>
                                                        </div>
