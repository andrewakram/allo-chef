...
@section('content')
            ...
            <div class="page-header">
                <div class="row">
                    <div class="col">
                        ...
                        @if(auth()->user()->hasPermissionTo('إضافة سائق'))
                                ...
                                <table class="display" id="basic-9">
                                    ...
                                    @foreach($users as $c)
                                        <tr id="main_cat_{{$c->id}}" onclick="myFunction({{$c->id}})"
                                            ...
                                            <td>
                                                @if(auth()->user()->hasPermissionTo('طلبات سائق'))
                                                <a href="{{asset('admin/users/orders/'.$c->id)}}">
                                                    <button title="طلبات السائق " class="btn btn-info">
                                                        <i class="fa fa-shopping-cart"></i>
                                                    </button>
                                                </a>
                                                @endif
                                                @if(auth()->user()->hasPermissionTo('تعديل سائق'))
                                                <button title="تعديل" type="button" class="btn btn-warning"
                                                        data-toggle="modal" data-target="#edit_{{$c->id}}">
                                                    <i class="fa fa-edit"></i>
                                                </button>
                                                @endif
                                                @if(auth()->user()->hasPermissionTo('تفعيل سائق'))
                                                    @if($c->suspend == 0)
                                                        <a href="{{route('editDriverStatus',$c->id)}}">
    ...
    <div class="modal fade" id="subCat" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">اضافة سائق</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form class="form-horizontal needs-validation was-validated" method="post"
                      action="{{route('drivers.store')}}" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <div class="modal-body">
