                ...
                <div class="row">
                    <div class="col">
                        ...
                        <div style="float: left">
                            @if(auth()->user()->hasPermissionTo('اضافة هوية'))
...
{{--                                <th scope="col">كود الدولة</th>--}}
                            ...
                            @foreach($countries as $c)
                                <tr id="main_cat_{{$c->id}}"   class="{{$c->active == 0 ? 'table-danger' :''}}">
                                    <td>{{$c->id}}</td>
                                    <td>{{$c->name}}</td>
...
{{--                                    <td>{{$c->code}}</td>--}}
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLabel">تعديل نوع هوية</h5>
                                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                </div>
                                                <form class="form-horizontal" method="post" action="{{route('editNational')}}" enctype="multipart/form-data">
                                                    {{csrf_field()}}
    ...
    <div class="modal fade" id="subCat" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">اضافة نوع هوية</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form class="form-horizontal needs-validation was-validated" method="post" action="{{route('nationals.store')}}" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <div class="modal-body">
