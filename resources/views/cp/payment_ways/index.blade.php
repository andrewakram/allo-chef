                ...
                <div class="row">
                    <div class="col">
                        ...
                        <div style="float: left">
                            {{--                            @if(auth()->user()->hasPermissionTo('اضافة مدينة'))--}}
...
{{--                                            <td>{{$c->country->name}}</td>--}}
                                            <td>
                                                @if(auth()->user()->hasPermissionTo('تعديل طرق الدفع'))
                                                    <button title="تعديل" type="button" class="btn btn-warning"
                                                            data-toggle="modal" data-target="#edit_{{$c->id}}">
                                                        <i class="fa fa-edit"></i>
                                                    </button>
                                                @endif
                                                @if(auth()->user()->hasPermissionTo('تفعيل طرق الدفع'))
                                                    @if($c->active == 1)
                                                        <a href="{{route('editPaymentWayStatus',$c->id)}}">
...
{{--                                                                @include('cp.layouts.error', ['input' => 'en_name'])--}}
    ...
    <div class="modal fade" id="subCat" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">اضافة طريقة دفع</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form class="form-horizontal needs-validation was-validated" method="post"
                      action="{{route('payment-ways.store')}}" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <div class="modal-body">
